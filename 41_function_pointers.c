...
/**
 * В стандартной библиотеке есть специальная функция qsort,
 * которая осуществляет сортировку данных по возрастанию относительно 
 * операции сравнения, которая передаётся в качестве параметра.
 * Сигнатура qsort:
 * void qsort(void *ptr, size_t count, size_t size, int (*comp) (void const *, void const *));
 * ptr - адрес начала массива данных
 * count - количество элементов в массиве
 * size - размер элемента в байтах
 * comp - указатель на функцию сравнения, которая получает адреса двух элементов, которые требуется сравзнить, 
 * а возвращает целое число: отрицательное, если первый элемент должен идти раньше второго, положительно, если 
 * первый элемент должен идти позже второго, ноль - элементы равны.
 * */

/**
 * Для примера напишем функцию, которая сравнивает два целых числа
 * */
